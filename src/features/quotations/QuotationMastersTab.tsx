...
const emptyTechnique: TechniqueInput = { code: "", name: "", unit_price: "", formula_type: "ONE_FACTOR", factor_1: "", factor_2: null, active: true, notes: null };
...
function OtherCostForm({ state, busy, error, onCancel, onSave, onChange }: { state: { id?: number; value: OtherCostInput }; busy: boolean; error: unknown; onCancel: () => void; onSave: () => void; onChange: (value: OtherCostInput) => void }) {
  const v = state.value;
  return <EditorPanel title={state.id ? "Editar otro gasto" : "Nuevo otro gasto"} busy={busy} valid={Boolean(v.code.trim() && v.name.trim() && v.unit_price)} error={error} onCancel={onCancel} onSave={onSave}><TextField label="Código" requirement="required" value={v.code} onChange={(code) => onChange({ ...v, code })} /><TextField label="Nombre" requirement="required" value={v.name} onChange={(name) => onChange({ ...v, name })} /><TextField label="Valor unitario" requirement="required" value={v.unit_price} onChange={(unit_price) => onChange({ ...v, unit_price })} inputMode="decimal" /><SelectField label="Tipo de cálculo" value={v.calculation_type} options={[{ value: "PER_DAY", label: "Por día" }, { value: "FIXED", label: "Fijo" }, { value: "PER_PIECE", label: "Por pieza" }]} onChange={(calculation_type: OtherCostCalculationType) => onChange({ ...v, calculation_type })} /><TextAreaField label="Notas" requirement="optional" value={v.notes} onChange={(notes) => onChange({ ...v, notes: notes || null })} /><ActiveToggle active={v.active} onChange={(active) => onChange({ ...v, active })} /></EditorPanel>;
}
